...
exports.descriptive = exports.array = exports.object = exports.date = exports.number = exports.string = exports.mixed = void 0;
...
exports.mixed = {
    ...
    oneOf: ({ label, values }) => (label ? label + "には" : "") + `以下のものが入力できます: ${values}`,
    notOneOf: ({ label, values }) => (label ? label + "には" : "") + `以下のものは入力できません: ${values}`,
    // From user's perspective, defined and default is essentially same.
    defined: ({ label }) => (label ? label + "は" : "") + "必須です",
};
exports.string = {
    length: ({ label, length }) => (label ? label + "は" : "") + `${length}文字である必要があります`,
    min: ({ label, min }) => (label ? label + "は" : "") + `${min}文字以上のみ入力できます`,
    max: ({ label, max }) => (label ? label + "は" : "") + `${max}文字まで入力できます`,
    matches: ({ label, regex }) => (label ? label + "は" : "") + `以下の形式である必要があります: ${regex}`,
    email: ({ label }) => (label ? label + "は" : "") + "正しいメールアドレスではありません",
...
exports.number = {
    ...
    moreThan: ({ label, more }) => (label ? label + "には" : "") + `${more}より大きい数のみ入力可能です`,
