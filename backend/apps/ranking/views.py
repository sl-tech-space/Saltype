...
class RankingView(APIView):
    ...
    def get_ranking(self, lang_id: int, diff_id: int, limit: int) -> list:
        ...
        通常のランキングを取得
        """
        return list(Score.objects
                    .filter(lang_id=lang_id, diff_id=diff_id) \
                    .select_related('user') \
                    .only('user__user_id', 'user__username', 'score') \
                    .order_by('-score')[:limit])

    def get_daily_ranking(self, lang_id: int, diff_id: int, date: str, limit: int) -> list:
        """ 
        指定日付に基づく日別ランキングを取得 
        """
        return list(Score.objects
                    .filter(lang_id=lang_id, diff_id=diff_id, created_at__date=date) \
                    .select_related('user') \
                    .only('user__user_id', 'user__username', 'score') \
                    .order_by('-score')[:limit])
