...
class ScoreByLangAndDiffView(APIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not lang_id or not diff_id:
            return Response({'error': 'lang_idとdiff_idが必須です'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        score_list = [
            {'user_id': score.user.user_id, 'username': score.user.username, 'score': score.score}
            for score in scores
        ]
