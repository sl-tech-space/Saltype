...
def testNo2_1_1(create_user):
    ...
    miss_data = [{'miss_char': 'a', 'miss_count': 5}, {'miss_char': 'b', 'miss_count': 3}]
    ...
    assert Miss.objects.filter(user_id=create_user.id, miss_char='a', miss_count=5).exists()
    assert Miss.objects.filter(user_id=create_user.id, miss_char='b', miss_count=3).exists()
